...
class ChurnPredictor:
    ...
    def _normalize_categorical_value(self, feature, value):
        if isinstance(value, str):  # Safeguard for strings
    ...
    def _prepare_input(self, customer_data):
        ...
        # Convert SeniorCitizen 'Yes'/'No' to 1/0 safely
        if 'SeniorCitizen' in customer_data:
            if isinstance(customer_data['SeniorCitizen'], str):  # Check if it is a string
                if customer_data['SeniorCitizen'].strip().lower() == 'yes':
                    customer_data['SeniorCitizen'] = 1
                elif customer_data['SeniorCitizen'].strip().lower() == 'no':
                    customer_data['SeniorCitizen'] = 0
                else:
                    raise ValueError(f"Invalid value '{customer_data['SeniorCitizen']}' for 'SeniorCitizen'")
    ...
    def predict(self, customer_data):
        ...
        if self.model is None or self.scaler is None:
            return {"status": "error", "error": "Model is not available"}
        try:
